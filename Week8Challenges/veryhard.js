...
function Programmer(language, busy) {
    ...
    this.completeTask = function () {
        this.busy = true;
        if (this.busy === true) {
            console.log(`${this.name} can not accept any ane tasks right now`);
        } else {
            console.log(`${this.name} can accept any new tasks right now.`);
        }
...
Person.prototype = Object.create(Programmer.prototype.constructor);
Person.prototype.constructor === Programmer;
var john = new Person ("John", "Back-End Developer", 26);
john.exercise();
john.fetchJob();
john.completeTask();
john.offerNewTask();
